...
class Chaoren():
    ...
    def GetUserInfo(self):
        datalist = Func.config.getKeysValue('captcha')
        for data in datalist:
            if data[0] == 'username':
                self.data['username'] = data[1]
            if data[0] == 'password':
                self.data['password'] = data[1]
            if data[0] == 'softid':
                self.data['softid'] = data[1]

    def get_left_point(self):
        try:
            r = self.s.post(
                'http://apib.sz789.net:88/GetUserInfo.ashx', self.data)
            return r.json()
    ...
    def recv_byte(self, imgdata):
        ...
        try:
            r = self.s.post(
                'http://apib.sz789.net:88/RecvByte.ashx', self.data)
            res = r.json()
            if res[u'info'] == -1:
                return False
            return r.json()
    ...
    def report_err(self, imgid):
        ...
        try:
            r = self.s.post(
                'http://apib.sz789.net:88/ReportError.ashx', self.data)
            return r.json()
        except requests.ConnectionError:
...
if __name__ == '__main__':
    client = Chaoren()
    client.data['username'] = 'mmmxyz'  # 修改为打码账号
    client.data['password'] = 'BX2KjvbE23r9Veq'  # 修改为打码密码
    # 查剩余验证码点数
